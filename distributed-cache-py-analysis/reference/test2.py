...
maxs = []
mins = []
ordered_data = sorted(indata, key=itemgetter('Region','City'))
for region, group in groupby(ordered_data, key=itemgetter('Region','City')):
...
for row in indata:
    ...
    city = row['City']
    sales = int(row['Sales'])
    dd_counts[region][city] += 1
    dd_sales[region][city] += sales
    dd_maxs[region][city] = max(dd_maxs[region][city], sales)
    dd_mins[region][city] = min(dd_mins[region][city], sales)
