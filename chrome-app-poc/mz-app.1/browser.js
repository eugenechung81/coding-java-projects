...
/*
  ...
  if (typeof(webview.setZoom) == "function" &&
    ...
    window.addEventListener('webkitfullscreenchange', handleFullscreen);
    webview.addEventListener('mozfullscreenchange', handleFullscreen);
    window.addEventListener('sizechanged', handleResize);
...
function doLayout() {
  ...
  //if(((windowHeight + 25) >= screen.height) && ((windowWidth) == screen.width)) {
  	var isFullscreen = chrome.app.window.current().isFullscreen();
  	var isMaximized = chrome.app.window.current().isMaximized();
  	var isMaxCheck = ((windowHeight + 25) >= screen.height) && ((windowWidth) == screen.width);
  	...
  	console.log("full:"+isFullscreen + "|max:" + isMaximized + "|max-check:"+ isMaxCheck + "|" + windowHeight + "|" + windowWidth);
  	...
  	else
  	{
  		...
  		if(isMaxCheck)
  		{
 			...
 			//setTimeout(chrome.app.window.current().fullscreen, 5);
  		...
  		else
  		{
 			  setTimeout(
			    function() {
			    	var x = 0.8 * screen.width;
			    	var y = 0.8 * screen.height;
			      //chrome.app.window.current().resizeTo(1500, 1000);
			      chrome.app.window.current().resizeTo(x, y);
			    },
			    5);
...
function handleLoadStart(event) {
  ...
  if(event.url == "http://www.mezocliq.com/") {
	setTimeout(chrome.app.window.current().fullscreen, 0);
  }
